...
function EventFilter({ events }: Props) {
  ...
  const isModalOpen = (modal_id: number): boolean => {
    return modalState.currentModal === modal_id || modalState.prevModal === modal_id;
  };
  ...
  return (
    <section className={cn('container')}>
      ...
      {isModalOpen(1) && <FilterSearchModal events={events} />}
      {isModalOpen(2) && <FilterTagModal />}
      {isModalOpen(3) && <FilterDateModal />}
    </section>
  );
}
