...
function EventNull() {
  ...
  const fetchTag = async () => {
    try {
      const context = `${jobGroupList?.join(', ')}, ${eventType ? eventType : ''}, ${location ? location : ''}, ${
        coast ? coast : ''
      }`;
      ...
      console.error('error fetchTag: ', err);
    }
  };
  useEffect(() => {
    fetchTag();
  ...
  return (
    <section className={cn('section__list')}>
      <section className={cn('container')}>
        ...
        <Image src={'/icon/none_event.svg'} alt="no event" priority={true} width={58} height={58} />
        <div className={cn('desc')}>추천태그로 검색해보세요</div>
        <div className={cn('tag__container')}>
          {tags?.map((tag) => {
            return <JobGroupTag key={tag.id} tagName={tag.tag_name} type="recommand" />;
          })}
        </div>
      </section>
    </section>
