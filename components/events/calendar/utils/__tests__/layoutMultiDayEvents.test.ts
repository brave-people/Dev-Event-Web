...
describe('layoutMultiDayEvents', () => {
  ...
  it('splits multi-day event crossing week boundary into two segments', () => {
    ...
    const events = [baseEvent({ id: 10, start_date_time: '2026-05-15T00:00:00', end_date_time: '2026-05-18T23:59:59' })];
    const result = layoutMultiDayEvents(events, matrix2026May);
    const segments = result.flat();
  ...
  it('includes outside cells when event spans month boundary', () => {
    ...
    const events = [baseEvent({ id: 11, start_date_time: '2026-05-30T00:00:00', end_date_time: '2026-06-01T23:59:59' })];
    const result = layoutMultiDayEvents(events, matrix2026May);
    const segments = result.flat();
  ...
  it('skips events with event_time_type=RECRUIT', () => {
    const events = [baseEvent({ id: 12, event_time_type: 'RECRUIT', start_date_time: '2026-05-12T00:00:00', end_date_time: '2026-05-12T23:59:59' })];
    const result = layoutMultiDayEvents(events, matrix2026May);
    expect(result.flat()).toHaveLength(0);
  });

  it('skips events with use_start_date_time_yn=N', () => {
    const events = [baseEvent({ id: 13, start_date_time: '2026-05-12T00:00:00', end_date_time: '2026-05-12T23:59:59', use_start_date_time_yn: 'N' })];
    const result = layoutMultiDayEvents(events, matrix2026May);
    expect(result.flat()).toHaveLength(0);
  });
  ...
  it('sorts overlapping events by display_sequence then start_date_time', () => {
    const events = [
      baseEvent({ id: 21, display_sequence: 2, start_date_time: '2026-05-12T10:00:00', end_date_time: '2026-05-12T12:00:00' }),
      baseEvent({ id: 22, display_sequence: 1, start_date_time: '2026-05-12T11:00:00', end_date_time: '2026-05-12T13:00:00' }),
    ];
    const result = layoutMultiDayEvents(events, matrix2026May);
    const segs = result.flat();
    expect(segs[0].eventId).toBe(22);
    expect(segs[1].eventId).toBe(21);
  });
});
